...
export function App() {
    ...
    async function hydrateFromCloud() {
      ...
      try {
        ...
        if (hasCloudData) {
          ...
          setAuthMessage("Cloud sync je aktivni.");
          window.setTimeout(() => {
          ...
          setAuthMessage("Cloud je prazdny. Zacinas s cistym uctem.");
          window.setTimeout(() => {
            isApplyingCloudStateRef.current = false;
          }, 0);
        }
        ...
        if (!isCancelled) {
          setAuthError(
            error instanceof Error ? error.message : "Automaticke nacteni cloudu selhalo.",
          );
        }
      } finally {
        if (!isCancelled) {
      ...
      try {
        ...
        if (!isCancelled) {
          ...
          setActiveTeamId((currentTeamId) =>
            currentTeamId && userTeams.some((team) => team.id === currentTeamId)
              ? currentTeamId
              : null,
          );
        }
    ...
    autoSyncTimeoutRef.current = window.setTimeout(() => {
      void (async () => {
        ...
        try {
          ...
          if (!sessionUser) {
            setAuthUser(null);
            throw new Error("Session neni aktivni. Prihlas se prosim znovu.");
          }
          ...
          setAuthError(
            error instanceof Error ? error.message : "Automaticke ulozeni do cloudu selhalo.",
          );
        } finally {
          setIsAutoSyncing(false);
        }
  ...
  async function handleCreateTeam(name: string, color?: string | null, description?: string | null) {
    if (!supabase || !authUser) {
      setAuthError("Pro vytvoreni tymu se nejdriv prihlas.");
      return null;
    }
    ...
    try {
      ...
      setAuthMessage("Tym " + team.name + " je pripraveny.");
      return team;
    } catch (error) {
      setAuthError(error instanceof Error ? error.message : "Vytvoreni tymu selhalo.");
      return null;
    } finally {
      setIsAuthLoading(false);
    }
  ...
  async function handleUploadLocalDataToCloud() {
    if (!supabase) {
      setAuthError("Supabase neni nakonfigurovany.");
      return;
    }
    ...
    try {
      ...
      setIsCloudReady(true);
      setAuthMessage(
        `Nahrano: ${result.lists} seznamu, ${result.tasks} ukolu, ${result.subtasks} podukolu, ${result.labels} stitku.`,
      );
    } catch (error) {
      setAuthError(error instanceof Error ? error.message : "Upload do cloudu selhal.");
    } finally {
      setIsCloudUploadLoading(false);
    }
  }
  ...
  async function handleDownloadCloudData() {
    ...
    try {
      ...
      setAuthMessage(
        `Nacteno z cloudu: ${cloudState.lists.filter((list) => !list.isSystem).length} seznamu a ${cloudState.tasks.length} ukolu.`,
      );
    } catch (error) {
      setAuthError(error instanceof Error ? error.message : "Nacteni z cloudu selhalo.");
    } finally {
      setIsCloudUploadLoading(false);
    }
  }
  ...
  async function handleSaveLocalChangesToCloud() {
    ...
    const shouldContinue = window.confirm(
      "Timto prepises cloud aktualni lokalni verzi. Pokracovat?",
    );
    ...
    try {
      ...
      setIsCloudReady(true);
      setAuthMessage(
        `Cloud ulozen: ${result.lists} seznamu, ${result.tasks} ukolu, ${result.subtasks} podukolu, ${result.labels} stitku.`,
      );
    } catch (error) {
      setAuthError(error instanceof Error ? error.message : "Ulozeni do cloudu selhalo.");
    } finally {
      setIsCloudUploadLoading(false);
    }
  }
  ...
  if (!isAuthSessionChecked) {
    return (
      <main className="auth-screen auth-screen--loading" aria-label="Nacitani DoNext">
        <section className="auth-screen__card">
          <div className="auth-screen__brand">
            <span aria-hidden="true">Do</span>
            <strong>DoNext</strong>
          </div>
          <p className="auth-screen__loading-copy">Nacitam ucet...</p>
        </section>
      </main>
    );
  }
