...
export function App() {
  ...
  useEffect(() => {
    ...
    async function hydrateUserProfile() {
      try {
        ...
        if (!isCancelled) {
          setAuthError(
            error instanceof Error ? error.message : "Nepodaøilo se naèíst profil uživatele.",
          );
        }
      }
    }
    ...
    async function hydrateFromCloud() {
      ...
      try {
        ...
        if (hasCloudData) {
          ...
          setAuthMessage("Cloud je prázdný. Zaèínáš s èistým úètem.");
          window.setTimeout(() => {
            isApplyingCloudStateRef.current = false;
          }, 0);
        }
    ...
    autoSyncTimeoutRef.current = window.setTimeout(() => {
      void (async () => {
        ...
        try {
          ...
          setAuthError(
            error instanceof Error ? error.message : "Automatické uložení do cloudu selhalo.",
          );
        } finally {
          setIsAutoSyncing(false);
        }
  ...
  async function handleSignUp(email: string, password: string) {
    ...
    if (error) {
      ...
      setAuthMessage(
        data.session
          ? "Úèet je vytvoøený a pøihlášený."
          : "Úèet je vytvoøený. Pokud Supabase vyžaduje potvrzení, zkontroluj e-mail.",
      );
    }
  ...
  async function handleCreateTeam(name: string, color?: string | null, description?: string | null) {
    if (!supabase || !authUser) {
      setAuthError("Pro vytvoøení týmu se nejdøív pøihlas.");
      return null;
    }
    ...
    try {
      ...
      setAuthMessage(`Tým ${team.name} je pøipravený.`);
      return team;
    } catch (error) {
      setAuthError(error instanceof Error ? error.message : "Vytvoøení týmu selhalo.");
      return null;
    } finally {
      setIsAuthLoading(false);
    }
  ...
  async function handleDeleteTeam(teamId: string) {
    if (!supabase || !authUser) {
      const error = new Error("Pro smazání týmu se nejdøív pøihlas.");
      setAuthError(error.message);
      throw error;
    }
  ...
  async function handleSaveLocalChangesToCloud() {
    ...
    try {
      ...
      setIsCloudReady(true);
      setAuthMessage(
        `Cloud uložen: ${result.lists} seznamù, ${result.tasks} úkolù, ${result.subtasks} podúkolù, ${result.labels} štítkù.`,
      );
    } catch (error) {
